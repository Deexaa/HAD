...
import json
import sys
import datasetting.custom_transforms as tr
...
class NYUv2(Dataset):
    ...
    """
    def __init__(self, root, mode, augmentation=True):
        self.mode = mode
        self.root = os.path.expanduser(root)
        self.augmentation = augmentation
        self.train_data_len = len(fnmatch.filter(os.listdir(self.root + '/train/image'), '*.npy'))
        self.test_data_len = len(fnmatch.filter(os.listdir(self.root + '/val/image'), '*.npy'))

    def _get_data(self, index, num_task):
    ...
    def __getitem__(self, i):
        ...
        labels = {"semantic":semantic.float(), 'depth': depth.float(), 'normal': normal.float()}
